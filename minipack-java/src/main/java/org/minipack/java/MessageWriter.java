...
/// format.
///
/// To create a new `MessageWriter`, call on of its [#of] methods. To write a value,
/// call one of the `write()` or `writeXYZ()` methods. To flush the underlying
/// [sink][MessageSink], call [#flush()]. To close this writer, call
/// [#close()].
public interface MessageWriter extends Closeable {
  ...
  ///
  /// A call to this method MUST be followed by `elementCount` calls that write the array's
  /// elements.
  void writeArrayHeader(int elementCount) throws IOException;

  /// Starts writing a map value with the given number of entries.
  ///
  /// A call to this method MUST be followed by `entryCount*2` calls that alternately write
  ...
  ///
  /// A call to this method MUST be followed by one or more calls to [#writePayload] that
  /// write exactly `byteCount` bytes in total.
  ///
  /// This method is a low-level alternative to [#write]. It can be useful in the following
  /// cases:
  ///
  ///     - The string to write is already available as UTF-8 byte sequence.
  ///   - Full control over conversion from `java.lang.String` to UTF-8 is required.
  ///
  ...
  ///
  /// A call to this method MUST be followed by one or more calls to [#writePayload] that
  /// write exactly `length` bytes in total.
  ...
  /// Writes an integer value that fits into a Java short. The given value is interpreted as
  // unsigned
  /// value.
